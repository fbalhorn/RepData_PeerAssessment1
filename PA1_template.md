...
This is a report document to present the analysis of the activity data set provided within the Coursera "reproducible research" course, week 2.

We will use the ggplot2 system for this excercise as well as the lubridate package to cope with dates:
...
At first, we load the dataset. It is assumed that the data is available in the current working directory:
...
The dataset summarizes as follows: 
...
1. and 2.: We can directly calculate the total number of steps per day and plot the result in a histogram:


```r
sumStepsPerDay<-tapply(activity_data$steps,activity_data$date,sum,na.rm=TRUE)
sumStepsPerDay
```
...
```r
qplot(sumStepsPerDay, binwidth = 500, geom="histogram") 
```

![plot of chunk unnamed-chunk-4](figure/unnamed-chunk-4-1.png) 

3.: We observe an accumulation of the step count per day just above 10000 steps. The corresponding mean and meadian od the steps taken per day are:


```r
meanStepsPerDay<-tapply(activity_data$steps,activity_data$date,mean,na.rm=TRUE)
medianStepsPerDay<-tapply(activity_data$steps,activity_data$date,median,na.rm=TRUE)
meanStepsPerDay
```
...
```r
#---
meanStepsPerDayTotal<-mean(sumStepsPerDay)
medianStepsPerDayTotal<-median(sumStepsPerDay)
#output shown in text below
```

The mean of all the steps taken par day is meanStepsPerDayTotal=9354.2295082, the median medianStepsPerDayTotal=10395.

## What is the average daily activity pattern?

Let's take a look at the time series plot of the 5 minute intervals. First, we need to calculate the average steps per day across the intervals:


```r
stepsPerInterval<-tapply(activity_data$steps,activity_data$interval,mean,na.rm=TRUE)
stepsPerInterval
```
...
1.: The plot of stepsPerInterval looks as follows:


```r
plot(stepsPerInterval, type='l',xlab="Interval Index",ylab="Steps Per Interval")
```
...
whichMax<-which.max(stepsPerInterval)
#output shown in text below
```

Note that the NA values have been removed in the above plot representation. 
2.: The 104th interval contains the maximum averaged steps per day of 206.1698113 steps.

## Imputing missing values

1.: The count of missing values in the steps column is:
...
2./3.: We can now replace the na values in the step column with the calculated mean value for the respective interval. If the value is still NaN, we insert a zero. The new dataset containing the missing data filled in following the mentioned strategy is saved in activity_data_corrected:


```r
#create copy of original dataset
activity_data_corrected<-activity_data

meanStepsPerInterval<-aggregate(activity_data$steps,list(activity_data$interval),mean,na.rm=TRUE)
rownames(meanStepsPerInterval)<-meanStepsPerInterval$Group.1

#replace values
i=0
for (i in 1:nrow(activity_data)){
  if(is.na(activity_data$steps[i])){
    activity_data_corrected$steps[i] <- subset(meanStepsPerInterval, Group.1 == activity_data[i,3],select=c(x))[[1]]
    }
}

head(activity_data,25)
```
...
```r
head(activity_data_corrected,25)
```
...
4.: The histogram now looks like:


```r
sumStepsPerDay_corrected<-tapply(activity_data_corrected$steps,activity_data_corrected$date,sum,na.rm=FALSE)
qplot(sumStepsPerDay_corrected, binwidth = 500, geom="histogram") 
```

![plot of chunk unnamed-chunk-10](figure/unnamed-chunk-10-1.png) 

```r
sumStepsPerDay_corrected
```
...
```r
meanStepsPerDay_corrected<-tapply(activity_data_corrected$steps,activity_data_corrected$date,mean,na.rm=FALSE)
medianStepsPerDay_corrected<-tapply(activity_data_corrected$steps,activity_data_corrected$date,median,na.rm=FALSE)
meanStepsPerDay_corrected
```
...
The mean values for the non-NA cases do not differ from those obtained without the corrections. Of course, the now corrected datasets show the identical value corresponding to the ideal set of averaged data. The mean values now contain data different from zero as the averaged dataset does not contain a single zero within its range. Therefore, the median must be greater than zero.

##Are there differences in activity patterns between weekdays and weekends?

1.: The factor $typeDay contains the information, if we're dealing with a weekday or a weekend-day:


```r
activity_data_corrected$typeDay <- ifelse(wday(as.Date(activity_data_corrected$date))>1 & wday(as.Date(activity_data_corrected$date))<7 ,"weekday", "weekend")
head(activity_data_corrected$typeDay)
```
...
2.: The results are displayed in the panel plot below.


```r
ggplot(activity_data_corrected, aes(x=interval,y=steps))+geom_line()+facet_grid(.~typeDay)
```

![plot of chunk unnamed-chunk-12](figure/unnamed-chunk-12-1.png) 

On weekdays, we see larger amount of performed steps during the early intervals per day, i.e. between interval 500 and 1000, compared to weekends. A possible interpretation might be, that people start their days later on weekends on average.
